 ...
 * Kept in sync with the ffmpeg extraction step. The film is a single phone in
 * a near-black void whose fit-score ring draws 0 → 100 across these frames.
 */
export const FRAME_COUNT = 101;

const framePath = (i: number) =>
  `/frames/frame_${String(i + 1).padStart(4, "0")}.jpg`;

/**
 * Scroll-scrubbed hero. A 380vh track pins a full-viewport <canvas>; scroll
 * progress maps to a frame index (0 → 100) and we redraw only when the index
 * changes, so the phone's fit-score ring fills from 0 to 100 as you scroll.
 * The same rAF loop fades the overlay copy away — no <video>, no scroll
 * listener, no useScroll. Reduced-motion users get the final full-ring frame.
 */
export function ScrollHero() {
  ...
  useEffect(() => {
    ...
    // Reduced motion: hold the final frame (the completed 100 ring), still.
    if (reduce) {
